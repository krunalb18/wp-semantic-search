...
class SemanticSearch_EmbeddingClient {
	...
	public function embed_batch(array $texts): array {
		$texts = array_values(array_filter(array_map('strval', $texts), function ($t) {
			return trim($t) !== '';
		}));
	...
	private function embed_batch_with_openai(array $texts): array {
		...
		$response = $this->request_with_retry($this->openai_api_url, array(
			'headers' => array(
	...
	private function embed_batch_with_gemini(array $texts): array {
		...
		$requests = array_map(function ($text) use ($model_name) {
			return array(
		...
		$response = $this->request_with_retry($url, array(
			'headers' => array(
				'Content-Type' => 'application/json',
			),
			'body' => wp_json_encode(array(
